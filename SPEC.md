...
**הפעלת מכונה נוספת (מחזור כפול):**
- כאשר נפתחת מטלת תלייה/ייבוש (= מכונה הכביסה סיימה) — הכרטיס בדשבורד מציג כפתור "הפעל מכונה נוספת 🧺" לצד כפתורי התביעה הרגילים
- לחיצה פותחת את מודל הפעלת המכונה הרגיל (בחירת אורך + בקשות דיירים) — בדיוק כמו הפעלה ראשונה
- מטלת הכביסה הנוצרת מוצגת כמטלה נפרדת חדשה; מטלת התלייה/ייבוש הקיימת ממשיכה כרגיל
- בנתיב ה-dry: האפשרות מופיעה גם בזמן אישור "הכנסתי למייבש" — כפתור "הפעל כביסה נוספת" בתחתית מודל בחירת אורך הייבוש
